...
class Game {
  ...
  make_gui() {
    ...
    const button_names = {
      Switch: {
        image: Icons.switch_icon,
        callback: () => {
          this.add_component(0);
        },
      },
      Button: {
        image: Icons.button_icon,
        callback: () => {
          this.add_component(1);
        },
      },
      Clock: {
        image: Icons.clock_icon,
        callback: () => {
          this.add_component(2);
        },
      },
      "True\nconstant": {
        image: Icons.true_constant_icon,
        callback: () => {
          this.add_component(3);
        },
      },
      "False\nconstant": {
        image: Icons.false_constant_icon,
        callback: () => {
          this.add_component(4);
        },
      },
      Light: {
        image: Icons.light_icon,
        callback: () => {
          this.add_component(5);
        },
      },
      "4 bit\ndisplay": {
        image: Icons.four_bit_display_icon,
        callback: () => {
          this.add_component(6);
        },
      },
      "8 bit\ndisplay": {
        image: Icons.eight_bit_display_icon,
        callback: () => {
          this.add_component(7);
        },
      },
      "Buffer gate": {
        image: Icons.buffer_gate_icon,
        callback: () => {
          this.add_component(8);
        },
      },
      "NOT gate": {
        image: Icons.not_gate_icon,
        callback: () => {
          this.add_component(9);
        },
      },
      "OR gate": {
        image: Icons.or_gate_icon,
        callback: () => {
          this.add_component(10);
        },
      },
      "NOR gate": {
        image: Icons.nor_gate_icon,
        callback: () => {
          this.add_component(11);
        },
      },
      "AND gate": {
        image: Icons.and_gate_icon,
        callback: () => {
          this.add_component(12);
        },
      },
      "NAND gate": {
        image: Icons.nand_gate_icon,
        callback: () => {
          this.add_component(13);
        },
      },
      "XOR gate": {
        image: Icons.xor_gate_icon,
        callback: () => {
          this.add_component(14);
        },
      },
      "XNOR gate": {
        image: Icons.xnor_gate_icon,
        callback: () => {
    ...
    this.rotate_button = create_button("Rotate", 0, 0, 0, 0, () => {
      this.rotate_selected_component(PI / 2);
    });
    this.delete_button = create_button("Delete", 0, 0, 0, 0, () => {
      this.destroy_selected_component();
    });
    this.copy_button = create_button("Copy", 0, 0, 0, 0, () => {
      this.copy_selection();
    });
    this.paste_button = create_button("Paste", 0, 0, 0, 0, () => {
      this.paste_selection();
    });
    this.menu_button = create_button("Menu", 0, 0, 0, 0, () => {
      this.show_menu();
    });
    ...
    if (make_vertical) {
      ...
      this.control_group.width =
        menu_button_width * this.control_group.widgets.length;
      this.control_group.height = menu_button_height;
      this.control_group.x =
        this.menu_group.x -
        5 -
        this.menu_line.stroke_weight -
        this.control_group.width;
      ...
      this.control_group.height =
        menu_button_height * this.control_group.widgets.length;
      this.control_group.x = this.side_group.x + menu_outside_pad;
      this.control_group.y =
        this.menu_group.y -
        5 -
        this.menu_line.stroke_weight -
        this.control_group.height;
  ...
  resize_gui() {
    this.graphics.resizeCanvas(
      windowWidth - window_diff,
      windowHeight - window_diff
    );
  ...
  add_component(c) {
    ...
    this.drag_component = new components[c](
      createVector((mouseX - camera.x) / zoom, (mouseY - camera.y) / zoom)
    );
  ...
  paste_selection() {
    ...
    for (const comp of this.copy_selected) {
      const new_comp = new comp.constructor(
        p5.Vector.add(comp.pos, createVector(30, 30))
      );
    ...
    for (const i in this.multi_selections) {
      ...
      const conn_out_pt_names =
        get_component_output_connect_point_names(old_comp);
      for (const name of conn_out_pt_names) {
        for (const conn of old_comp[name].connections) {
          ...
          const to_new_comp =
            this.multi_selections[
              this.copy_selected.indexOf(conn.to_point.parent)
            ];
          ...
          try {
            this.items[1].push(
              make_connection(new_comp[name], to_new_comp[to_new_comp_name])
            );
          } catch {
    ...
    if (mouseButton === LEFT) {
      ...
      if (this.drag_component !== hovering[0]) {
        if (keyIsDown(16) && hovering[0] instanceof Component) {
          const index_multi_selections = this.multi_selections.indexOf(
            hovering[0]
          );
  ...
  on_mouse_release() {
    ...
    if (this.drag_connection != undefined) {
      ...
      if (
        this.drag_connection instanceof ConnectionOutPoint &&
  ...
  make_testing_objects() {
    ...
    this.connections.push(
      make_connection(this.components[0].output1, this.components[1].input1)
    );
    this.connections.push(
      make_connection(this.components[0].output1, this.components[2].input1)
    );
    // -- connect one switch to another light
    this.connections.push(
      make_connection(this.components[3].output1, this.components[4].input1)
    );
    // -- connect a switch to a light, changing it's connection
    this.connections.push(
      make_connection(this.components[3].output1, this.components[2].input1)
    );
    // buffer gate
    ...
    this.connections.push(
      make_connection(this.components[6].output1, this.components[5].input1)
    );
    // -- light bulbs from buffer gate
    this.components.push(new Light(createVector(400, 10)));
    this.connections.push(
      make_connection(this.components[5].output1, this.components[7].input1)
    );
    this.components.push(new Light(createVector(450, 10)));
    this.connections.push(
      make_connection(this.components[5].output1, this.components[8].input1)
    );
    // -- not gate
    this.components.push(new NotGate(createVector(400, 80)));
    // -- connection from switch to not gate
    this.connections.push(
      make_connection(this.components[6].output1, this.components[9].input1)
    );
    // -- light bulb from not gate
    this.components.push(new Light(createVector(500, 10)));
    this.connections.push(
      make_connection(this.components[9].output1, this.components[10].input1)
    );
    // not gate
    this.components.push(new NotGate(createVector(300, 160)));
    // -- connect not gate to self to test
    this.connections.push(
      make_connection(this.components[11].output1, this.components[11].input1)
    );
    // and gate
    ...
    this.connections.push(
      make_connection(this.components[13].output1, this.components[12].input1)
    );
    this.connections.push(
      make_connection(this.components[14].output1, this.components[12].input2)
    );
    // -- connect and gate to light
    this.components.push(new Light(createVector(380, 200)));
    this.connections.push(
      make_connection(this.components[12].output1, this.components[15].input1)
    );
    // nand gate
    ...
    // -- connect switches to and gate
    this.connections.push(
      make_connection(this.components[17].output1, this.components[16].input1)
    );
    this.connections.push(
      make_connection(this.components[18].output1, this.components[16].input2)
    );
    // -- connect nand gate to lights
    this.components.push(new Light(createVector(600, 200)));
    this.connections.push(
      make_connection(this.components[16].output1, this.components[19].input1)
    );
    this.components.push(new Light(createVector(650, 200)));
    this.connections.push(
      make_connection(this.components[16].output1, this.components[20].input1)
    );
    // or gate
    ...
    // -- connect switches to and gate
    this.connections.push(
      make_connection(this.components[22].output1, this.components[21].input1)
    );
    this.connections.push(
      make_connection(this.components[23].output1, this.components[21].input2)
    );
    ...
    this.connections.push(
      make_connection(this.components[24].output1, this.components[25].input1)
    );
    ...
    this.connections.push(
      make_connection(this.components[26].output1, this.components[24].input1)
    );
    this.connections.push(
      make_connection(this.components[27].output1, this.components[24].input2)
    );
    this.connections.push(
      make_connection(this.components[28].output1, this.components[25].input2)
    );
    // -- attach not gate to output
    this.components.push(new NotGate(createVector(600, 450)));
    // -- connect or gate output to not gate
    this.connections.push(
      make_connection(this.components[25].output1, this.components[29].input1)
    );
    // -- light bulb for output
    this.components.push(new Light(createVector(600, 350)));
    this.connections.push(
      make_connection(this.components[29].output1, this.components[30].input1)
    );
    // nor gate
    ...
    this.connections.push(
      make_connection(this.components[32].output1, this.components[31].input1)
    );
    this.connections.push(
      make_connection(this.components[33].output1, this.components[31].input2)
    );
    ...
    this.connections.push(
      make_connection(this.components[36].output1, this.components[35].input1)
    );
    this.connections.push(
      make_connection(this.components[35].output1, this.components[34].input1)
    );
    this.connections.push(
      make_connection(this.components[34].output1, this.components[36].input1)
    );
    this.components.push(new Switch(createVector(200, 400)));
    this.connections.push(
      make_connection(this.components[37].output1, this.components[35].input2)
    );
    // xor gate
    ...
    this.connections.push(
      make_connection(this.components[39].output1, this.components[38].input1)
    );
    this.connections.push(
      make_connection(this.components[40].output1, this.components[38].input2)
    );
    // xnor gate
    ...
    this.connections.push(
      make_connection(this.components[42].output1, this.components[41].input1)
    );
    this.connections.push(
      make_connection(this.components[43].output1, this.components[41].input2)
    );
    // clock signal
    ...
    this.connections.push(
      make_connection(this.components[44].output1, this.components[46].input1)
    );
    this.connections.push(
      make_connection(this.components[45].output1, this.components[46].input2)
    );
    // constant true
    ...
    this.connections.push(
      make_connection(this.components[50].output1, this.components[49].input1)
    );
    this.connections.push(
      make_connection(this.components[51].output1, this.components[49].input2)
    );
    this.connections.push(
      make_connection(this.components[52].output1, this.components[49].input3)
    );
    this.connections.push(
      make_connection(this.components[53].output1, this.components[49].input4)
    );
    // 8 bit digit
    ...
    for (let i = 0; i < 8; i++) {
      this.connections.push(
        make_connection(
          this.components[55 + i].output1,
          this.components[54]["input" + (i + 1)]
        )
      );
    }
  ...
  update() {
    const update =
      this.update_cycles_left === -1 || this.update_cycles_left > 0;
    ...
    if (pls_align) {
      this.set_specific_button.clickable.x =
        this.selected_component.pos.x * zoom +
        camera.x -
        this.set_specific_button.clickable.width / 1.5;
      this.set_specific_button.clickable.y =
        this.selected_component.pos.y * zoom +
        camera.y -
        this.set_specific_button.clickable.height / 1.5;
      this.set_specific_button.clickable.x = Math.max(
        this.set_specific_button.clickable.x,
        0
      );
      this.set_specific_button.clickable.y = Math.max(
        this.set_specific_button.clickable.y,
        0
      );
    }
  ...
  draw_grid(cell_size, cam) {
    ...
    for (
      ...
      this.graphics.line(
        -shift.x,
        y,
        (width + add_some) / zoom + (shift_x - shift.x),
        y
      );
    }
    for (
      let x = shift_x - shift.x;
  ...
  draw() {
    ...
    for (const group of this.items) {
      for (const item of group) {
        if (hovering_on_button() && this.drag_component === item) {
          ...
          item === this.selected_component ||
          this.multi_selections.includes(item)
        ) {
    ...
    if (show_mouse_coords) {
      ...
      textSize(12 / zoom);
      let string =
        Math.round((mouseX - camera.x) / zoom) +
        ", " +
        Math.round((mouseY - camera.y) / zoom);
      if (zoom !== 1) {
        string += " at " + Math.round(zoom * 10) / 10 + "x";
      }
      string +=
        " at frame " +
        Math.round(frame_millis) +
        " sub " +
        Math.round(frame_sub);
