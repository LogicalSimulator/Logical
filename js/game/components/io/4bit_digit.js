...
class FourBitDigit extends Component {
  constructor(pos) {
    ...
    this.input1 = new ConnectionInPoint(
      this,
      createVector(four_bit_digit_width / 2, four_bit_digit_height * 0.2),
      createVector(-(component_height * 0.75), 0),
      "input1_state"
    );
    this.input2_state = false;
    this.input2 = new ConnectionInPoint(
      this,
      createVector(four_bit_digit_width / 2, four_bit_digit_height * 0.4),
      createVector(-(component_height * 0.75), 0),
      "input2_state"
    );
    ...
    this.input3 = new ConnectionInPoint(
      this,
      createVector(four_bit_digit_width / 2, four_bit_digit_height * 0.6),
      createVector(-(component_height * 0.75), 0),
      "input3_state"
    );
    ...
    this.input4 = new ConnectionInPoint(
      this,
      createVector(four_bit_digit_width / 2, four_bit_digit_height * 0.8),
      createVector(-(component_height * 0.75), 0),
      "input4_state"
    );
  ...
  draw(graphics, outline) {
    ...
    graphics.rect(
      four_bit_digit_x,
      four_bit_digit_y,
      four_bit_digit_width,
      four_bit_digit_height
    );

    graphics.stroke(four_bit_digit_stroke);
    graphics.fill(
      this.display_digit === "0"
        ? four_bit_digit_stroke
        : four_bit_digit_powered_fill
    );
    graphics.textAlign(CENTER, CENTER);
    graphics.textSize(
      get_text_size(
        this.display_digit,
        four_bit_digit_width * 0.75,
        four_bit_digit_height * 0.75
      )
    );
    graphics.text(
      this.display_digit,
      four_bit_center.x,
      four_bit_center.y + four_bit_digit_height * 0.05
    );
